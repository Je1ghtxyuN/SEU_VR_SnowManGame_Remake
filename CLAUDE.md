...
Unity MCP server (`com.coplaydev.unity-mcp`) is installed. After starting the Unity Editor, run the MCP server from Unity's menu or console to enable Claude Code to interact with the editor (scene manipulation, console reading, script editing, etc.).
